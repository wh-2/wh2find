InfluxDB::Rails.configure do |config|
  ...
  config.client.database = ENV['INFLUX_DB']
  config.client.hosts = ENV['INFLUX_HOST']
  config.client.port = ENV['INFLUX_PORT']

  ## If you've setup user authentication (and activated it in the server
  ## config), you need to configure the credentials here.
  if ENV['INFLUX_USER'] && ENV['INFLUX_PASS']
    config.client.username = ENV['INFLUX_USER']
    config.client.password = ENV['INFLUX_PASS']
  end
